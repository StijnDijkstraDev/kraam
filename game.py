...
Game: TypeAlias = tuple[set[VertId], dict[VertId, Priority], set[VertId], dict[VertId, set[VertId]], dict[VertId, set[VertId]]]

def parse_game(filename: str) -> Game:
    ...
    file.close()
    return (vertices, priority, owned, outEdges, incEdges)
    
def is_valid_subgame(parent: Game, cVertices: set[VertId]) -> bool:
    (pVertices, pPriority, pOwned, pOutEdges, pIncEdges) = parent
...
def realise_subgame(game: Game, cVertices: set[VertId]) -> Game:
    (pVertices, pPriority, pOwned, pOutEdges, pIncEdges) = game
    ...
    return (cVertices, cPriority, cOwned, cOutEdges, cIncEdges)


def flatten_game(game: Game, startvertex: VertId) -> tuple[Game, VertId]:
    (oldVertices, oldPriority, oldOwned, oldOutEdges, oldIncEdges) = game
    
    # map old to new vertices
    i = 0
    ...
    return ((newVertices, newPriority, newOwned, newOutEdges, newIncEdges), mapping[startvertex])


def export_to_file(game: Game, filename: str) -> None:
    (vertices, priority, owned, outEdges, incEdges) = game
